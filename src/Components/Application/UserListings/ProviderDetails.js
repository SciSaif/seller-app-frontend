...
const ProviderDetails = ({ isFromUserListing = false }) => {
    ...
    const getOrgDetails = async (id) => {
        try {
            ...
            setBankDetails({
                ...
                cancelledCheque:
                    res?.providerDetail?.bankDetails?.cancelledCheque?.url,
            });
            ...
            let storeData = {
                email:
                    res.providerDetail.storeDetails?.supportDetails.email || "",
                mobile:
                    res.providerDetail.storeDetails?.supportDetails.mobile ||
                    "",
                ...
                location_availability: res.providerDetail.storeDetails
                    ? res.providerDetail.storeDetails
                          .locationAvailabilityPANIndia == true
                        ? "pan_india"
                        : "city"
                    : "",
                ...
                country:
                    res.providerDetail?.storeDetails?.address?.country || "",
                state: res.providerDetail?.storeDetails?.address?.state || "",
                city: res.providerDetail?.storeDetails?.address.city || "",
                address_city:
                    res.providerDetail?.storeDetails?.address.city || "",
                building:
                    res.providerDetail?.storeDetails?.address?.building || "",
                area_code:
                    res.providerDetail?.storeDetails?.address?.area_code || "",
                locality:
                    res.providerDetail?.storeDetails?.address?.locality || "",
                ...
                logisticsBppId:
                    res?.providerDetail?.storeDetails?.logisticsBppId || "",
                logisticsDeliveryType:
                    res?.providerDetail?.storeDetails?.logisticsDeliveryType ||
                    "",
            };
            ...
            const storeTimingDetails =
                res?.providerDetail?.storeDetails?.storeTiming;
    ...
    const validate = () => {
            ...
            supportedFulfillments.delivery !== false
                ? fulfillmentDetails.deliveryDetails?.deliveryEmail?.trim() ===
                  ""
                    ...
                    : !isEmailValid(
                          fulfillmentDetails.deliveryDetails?.deliveryEmail
                      )
                    ? "Please enter a valid email address"
                    : ""
                : "";
            ...
            supportedFulfillments.delivery !== false
                ? fulfillmentDetails.deliveryDetails?.deliveryMobile?.trim() ===
                  ""
                    ...
                    : !isPhoneNoValid(
                          fulfillmentDetails.deliveryDetails?.deliveryMobile
                      )
                    ? "Please enter a valid mobile number"
                    : ""
                : "";
            ...
            supportedFulfillments.selfPickup !== false
                ? fulfillmentDetails.selfPickupDetails?.selfPickupEmail?.trim() ===
                  ""
                    ? "Delivery Email is required"
                    : !isEmailValid(
                          fulfillmentDetails.selfPickupDetails?.selfPickupEmail
                      )
                    ? "Please enter a valid email address"
                    : ""
                : "";
        ...
        if (supportedFulfillments.deliveryAndSelfPickup) {
            ...
            const deliveryEmail =
                deliveryAndSelfPickupDetails.deliveryEmail?.trim();
    ...
    const anyChangeInData = () => {
        ...
        let is_form_updated = !areObjectsEqual(
            storeDetails,
            defaultStoreDetails
        );
    ...
    const getFulfillmentsPayloadFormat = () => {
        ...
        if (supportedFulfillments.deliveryAndSelfPickup) {
            let deliveryAndSelfPickupDetails = {
                ...
                contact: {
                    delivery: {
                        email: fulfillmentDetails.deliveryAndSelfPickupDetails
                            .deliveryEmail,
                        phone: fulfillmentDetails.deliveryAndSelfPickupDetails
                            .deliveryMobile,
                    },
                    pickup: {
                        email: fulfillmentDetails.deliveryAndSelfPickupDetails
    ...
    return (
        <div>
            <div className="container mx-auto my-8">
                <div>
                    <div
                        className="w-full h-full px-4 py-4 bg-white rounded-md scrollbar-hidden"
                        style={{
                            minHeight: "95%",
                            maxHeight: "100%",
                            overflow: "auto",
                    ...
                    >
                        <div className="w-10/12 m-auto md:w-3/4 h-max">
                            <div className="flex justify-between">
                                <BackNavigationButton
                                    onClick={() => {
                                        userRole == "Super Admin"
                                            ? navigate(
                                                  "/application/user-listings?view=provider"
                                              )
                                            : navigate(
                                ...
                                {isFromUserListing && (
                                    <div className="flex flex-row gap-2 ">
                                        <Link
                                            to={`/application/inventory?org_id=${params?.id}`}
                                        >
                                            <MyButton
                                        ...
                                        <Link
                                            to={`/application/orders?org_id=${params?.id}`}
                                        >
                                            <MyButton
                                                className="!text-black"
                                                title="Orders"
                            ...
                            <p className="mb-4 text-2xl font-semibold mt-14">
                                KYC Details
                            </p>
                            {kycFields.map((item) => (
                                <RenderInput
                            ...
                            {!isFromUserListing && (
                                <>
                                    ...
                                    <p className="mb-2 text-2xl font-semibold mt-14">
                                        Store Timing
                                    </p>
                                    <RenderInput
                                        item={{
