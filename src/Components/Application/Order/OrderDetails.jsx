...
const OrderDetails = () => {
  ...
  // read query from url called ord_id
  const [searchParams, setSearchParams] = useSearchParams();
  const org_id = searchParams.get("org_id");
  let isAdmin = false;
  if (org_id) isAdmin = true;
  ...
  if (price_breakup) {
    ...
    const delivery_charges_tax_object = price_breakup?.filter(
      (b) =>
        b["@ondc/org/title_type"] === "tax" && b.title === "Delivery charges"
    );
  ...
  const handleOrderStatusChange = (order_uuid) => {
  ...
  const handleFulfillmentStatusChange = (order_uuid, status) => {
    setloading({ ...loading, accept_order_loading: true });
    const url = `/api/v1/orders/${order_uuid}/fulfillment_status`;
    postCall(url, {
      status: status,
    })
      .then((resp) => {
        cogoToast.success("Fulfillment status changed successfully!");
        handleClose();
        ...
        for (const fulfillment of orderData.fulfillments) {
          if (fulfillment.type === "Delivery") {
            fulfillment.state.descriptor.code = status;
          }
        }
    ...
    if (
      ...
      return (
        <div style={{ display: "flex", direction: "row", gap: "8px" }}>
          ...
          order_details?.state === "In-progress" ? (
            <>
              <Tooltip title="Update fulfillment status">
                  ...
                  <Stack direction="column" spacing={2}>
                    <FormControl fullWidth>
                      ...
                      <Select
                        ...
                        label="Select Fulfillment Status"
                        onChange={(e) => {
          ...
          {order_details?.state === "Created" ? (
            <>
              <Button
                ...
                variant="contained"
                onClick={() => handleOrderStatusChange(order_details?._id)}
                disabled={
  ...
  return (
    <div className="container mx-auto my-8">
      <BackNavigationButton
        onClick={() =>
          navigate(`/application/orders${isAdmin ? `?org_id=${org_id}` : ""}`)
        }
      />
      <div className="flex flex-col">
        <div className={`${cardClass} my-4 p-4`}>
          <div className="flex justify-between">
            <p className="mb-2 text-lg font-semibold">Order Summary</p>
          ...
          <div className="flex justify-between mt-3 mb-3">
            ...
            <p className="font-normal text-md">{order?.billing?.name}</p>
          </div>
          ...
          <div className="flex justify-between mt-3">
            <p className="text-base font-normal">Total Packing Charge</p>
            <p className="text-base font-normal">
          ...
          <div className="flex justify-between pt-2 mt-3 border-t ">
            <p className="text-sm font-normal">Buyer Finder Fee %</p>
            <p className="text-sm font-normal">
              {buyer_finder_fee_percent ? buyer_finder_fee_percent + "%" : "-"}
            </p>
          </div>
          <div className="flex justify-between mt-3 ">
            <p className="text-sm font-normal">Buyer Finder Fee Amount</p>
            <p className="text-sm font-normal">
        ...
        <div className={`${cardClass} my-4 p-4`}>
          <div className="flex h-full">
            <div className="flex-1 mr-8">
              <p className="mb-2 text-lg font-semibold">Delivery Address</p>
            ...
            <div className="flex-1 ml-8">
              <p className="mb-2 text-lg font-semibold">Billing Address</p>
              <div className="flex flex-col justify-between my-3">
                <p className="text-lg font-medium">
        ...
        <div className="flex justify-between">
          <div className="w-full">
            <div className={`${cardClass} my-4 p-4`}>
              <p className="mb-2 text-lg font-semibold">Customer Details</p>
              ...
              <div className="flex items-center">
                ...
                <p className="text-sm font-medium">
                  {delivery_info?.end?.contact?.email || "NA"}
                </p>
              </div>
            </div>
          </div>
...
const OrderItemsSummaryCard = (props) => {
  ...
  const [fulfillment_state, setFulfillmentStatus] = React.useState(null);
  ...
  useEffect(() => {
    ...
    const current_fstate =
      props?.order?.fulfillments[0]?.state?.descriptor?.code;
    setFulfillmentStatus(current_fstate);

    console.log("order in summary card", props?.order);
    if (props?.order) {
      const current_items = Object.values(parseQuoteToGetItems(props?.order));
      setOrderItems(current_items);
    }
  }, [props?.order]);
      ...
      <>
        <Tooltip title="Action">
          <Button
            ...
            disabled={
              fulfillment_state !== "Pending" || props.row.quantity == "0"
            }
          >
            <MoreVert />
          </Button>
  ...
  const renderItem = (item, fulfillment_state) => {
    return cols.map((col) => {
      return (
          ...
          <Table>
            ...
            <TableBody>
              {order_items?.map((item) => {
                console.log("item", item, fulfillment_state);
                return (
                  <ExpandableTableRow
                    sx={{
                      "& > *": {
                  ...
                  >
                    {renderItem(item, fulfillment_state)}
                  </ExpandableTableRow>
                );
              })}
