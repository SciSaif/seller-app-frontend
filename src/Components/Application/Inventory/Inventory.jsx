...
export default function Inventory() {
    ...
    const getProducts = async () => {
        try {
            const res = await cancellablePromise(
                getCall(
                    `/api/v1/products?limit=${rowsPerPage}&offset=${page}${
    ...
    useEffect(() => {
        ...
        // if org_id is present, then its for admin
        if (org_id) {
            getOrgDetails(org_id).then((org) => {
                if (isObjEmpty(org.storeDetails)) navigate("/");
            });
                ...
                if (u.isSystemGeneratedPassword) navigate("/initial-steps");
                    ...
                    if (u.role.name == "Organization Admin") {
                        getOrgDetails(u.organization).then((org) => {
                            if (isObjEmpty(org.storeDetails))
                        ...
                        <div className="flex">
                            <div style={{ marginRight: 15 }}>
                                <Button
                                    ...
                                    title="Bulk upload"
                                    onClick={() =>
                                        navigate("/application/bulk-upload")
                                    }
                                />
                            </div>
                            <div style={{ marginRight: 15 }}>
